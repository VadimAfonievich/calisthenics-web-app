...
func (s *Service) List(ctx context.Context, difficulty, muscle, movement, equipment, tag, search string) ([]Exercise, error) {
	...
	rows, e := s.pool.Query(ctx, `SELECT e.id::text,e.name,e.description,e.instructions,e.common_mistakes,e.difficulty,e.muscle_groups,e.equipment,e.tags,e.coach_tips,COALESCE(m.url,''),dm.url,dm.type,dm.mime_type,dm.thumbnail_url FROM exercises e LEFT JOIN media_assets m ON m.id=e.cover_media_id LEFT JOIN media_assets dm ON dm.id=e.demo_media_id WHERE e.status='published' AND (e.tenant_id=$7::uuid OR (e.tenant_id IS NULL AND e.standard_key IS NOT NULL)) AND ($1='' OR e.difficulty=$1) AND ($2='' OR $2=ANY(e.muscle_groups)) AND ($3='' OR e.movement_type=$3) AND ($4='' OR $4=ANY(e.equipment)) AND ($5='' OR $5=ANY(e.tags)) AND ($6='' OR e.name ILIKE '%'||$6||'%' OR e.description ILIKE '%'||$6||'%') ORDER BY e.difficulty,e.name`, difficulty, muscle, movement, equipment, tag, strings.TrimSpace(search), tenantArg(tenant))
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *Service) Get(ctx context.Context, id string) (Exercise, error) {
	...
	var x Exercise
	var u, kind, mime, poster *string
	e := s.pool.QueryRow(ctx, `SELECT e.id::text,e.name,e.description,e.instructions,e.common_mistakes,e.difficulty,e.muscle_groups,e.equipment,e.tags,e.coach_tips,COALESCE(m.url,''),dm.url,dm.type,dm.mime_type,dm.thumbnail_url FROM exercises e LEFT JOIN media_assets m ON m.id=e.cover_media_id LEFT JOIN media_assets dm ON dm.id=e.demo_media_id WHERE e.id=$1::uuid AND e.status='published' AND (e.tenant_id=$2::uuid OR (e.tenant_id IS NULL AND e.standard_key IS NOT NULL))`, id, tenantArg(tenant)).Scan(&x.ID, &x.Name, &x.Description, &x.Instructions, &x.CommonMistakes, &x.Difficulty, &x.MuscleGroups, &x.Equipment, &x.Tags, &x.CoachTips, &x.CoverMediaURL, &u, &kind, &mime, &poster)
	if e == nil && u != nil {
		x.DemoMedia = &DemoMedia{URL: *u, Type: *kind, MIMEType: *mime}
		if poster != nil {
			x.DemoMedia.PosterURL = *poster
