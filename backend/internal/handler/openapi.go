...
paths:
  ...
  /api/v1/me:
    get:
      ...
      responses:
        '200':
          description: Current user, authoritative role, and permitted UI modes
          content:
            application/json:
              schema:
                type: object
                properties:
                  user:
                    type: object
                    properties:
                      role: {type: string, enum: [user, coach, admin, super_admin]}
                      available_modes:
                        type: array
                        items: {type: string, enum: [student, coach]}
