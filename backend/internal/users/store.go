...
func (s *Store) GetByID(ctx context.Context, id string) (User, error) {
	var user User
	err := s.pool.QueryRow(ctx, `SELECT u.id::text, u.telegram_id, COALESCE(u.username, ''), u.first_name, COALESCE(u.last_name, ''), COALESCE(u.photo_url, ''), p.display_name, p.level, p.xp, p.current_streak, p.longest_streak, p.preferred_difficulty, p.timezone
FROM users u JOIN profiles p ON p.user_id = u.id WHERE u.id = $1::uuid`, id).Scan(&user.ID, &user.TelegramID, &user.Username, &user.FirstName, &user.LastName, &user.PhotoURL, &user.DisplayName, &user.Level, &user.XP, &user.CurrentStreak, &user.LongestStreak, &user.PreferredDifficulty, &user.Timezone)
	if err != nil {
		return User{}, err
	}
	return user, nil
