...
The workout editor follows one mobile-first flow: basic fields, exercise selection, per-exercise configuration, ordering, warmup, preview, and explicit draft or publish actions. It is designed for Telegram widths from 320 to 414 px with stacked cards, large touch targets, a full-height exercise picker sheet, and sticky save actions. Coaches never select program levels or see relation UUIDs in this editor because programs compose standalone workouts.

The exercise picker uses the lightweight Coach options response and does not load demo video payloads. Search covers name, description, and tags; filters cover difficulty, repetitions/time, equipment, direction tags, muscle groups, and system versus coach ownership. Cards mark system exercises as «Стандартное» and owned exercises as «Моё». Multi-select stays open until confirmation, shows the selected count, and prevents duplicate composition rows. Tapping a card opens an inline detail sheet with description, technique, common mistakes, tips, muscles, equipment, and demo availability. A link to the exercise editor is available when the catalog has no suitable result.

Each selected exercise is configured as repetitions XOR duration, with sets, rest-after-set, optional student notes, manual numeric input, and quick presets. Empty numeric text remains empty while editing and is validated only when saving. Up/down controls persist contiguous `sort_order`; removal affects only the workout composition, never the exercise library. The UI duration estimate is advisory and can be copied into `estimated_minutes` or overridden by the coach.

Warmup selection includes only published warmup workouts and displays duration plus exercise count. Turning warmup off persists `warmup_enabled=false`; leaving the concrete selection empty uses the existing system-default fallback. A selected warmup can be previewed without leaving the editor. Workout preview shows the learner-facing title, description, difficulty, duration, warmup, and ordered set outline.

Draft save accepts missing publish-only title/description, while retaining database-safe category, difficulty, duration, and composition constraints. Publish requires title, description, at least one valid exercise, positive sets, repetitions XOR duration, non-negative rest, published dependencies, and a valid published warmup. The editor presents all detectable issues in a validation summary and moves it into view. Backend lifecycle validation remains authoritative for ownership and dependency conflicts. Duplicate produces a new owned draft with composition, notes, order, and warmup settings; archive continues to reject workouts referenced by published programs.
