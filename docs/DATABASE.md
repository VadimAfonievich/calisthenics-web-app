...
# Coach content ownership and lifecycle

Migration `000007_coach_studio` extends `admin_users` with the `coach` role and adds nullable `owner_user_id`, lifecycle status, cover media, and publication audit columns to content. Null owners identify backward-compatible system seed content. Lessons add validated JSONB blocks; `media_assets` stores metadata only. Composite owner/status indexes support studio lists.
