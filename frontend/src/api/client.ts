...
export async function api<T>(path: string, options: RequestInit = {}, accessToken?: string): Promise<T> {
  const tenantSlug=typeof localStorage==='undefined'?null:localStorage.getItem('calisthenics_tenant_slug')
  const response = await fetch(buildAPIURL(path), { ...options, headers: { 'Content-Type': 'application/json', ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}), ...(accessToken&&tenantSlug?{'X-Tenant-Slug':tenantSlug}:{}), ...options.headers } })
