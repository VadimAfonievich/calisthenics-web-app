...
import {complete,getSession,listWorkouts,normalizeWorkoutDuration,saveSet} from './workouts'
...
describe('workout API',()=>{
  ...
  it('caps stale session duration at the API maximum when completing',async()=>{const fetchMock=vi.fn().mockResolvedValue({ok:true,status:200,json:async()=>({session:{}})});vi.stubGlobal('fetch',fetchMock);expect(normalizeWorkoutDuration(346800)).toBe(43200);await complete('token',id,346800);expect(fetchMock).toHaveBeenCalledWith(expect.stringMatching(/duration_seconds=43200$/),expect.objectContaining({method:'POST'}))})
