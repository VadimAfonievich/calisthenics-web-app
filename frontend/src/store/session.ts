...
import { authenticateTelegram, type AppMode, type CurrentUser } from '../api/auth'
...
export const useSessionStore = create<SessionState>((set) => ({
  ...
  async bootstrap(initData) {
    if (!initData) { set((state) => ({ status: 'demo', user: demoUser, appMode: 'student', diagnostics: { ...state.diagnostics, initDataPresent: false, initDataLength: 0 } })); return }
