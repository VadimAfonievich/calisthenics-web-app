...
describe('Voice Coach choreography',()=>{
  ...
  it('keeps same timed exercise as rest without next-exercise phrase',()=>{const{spoken}=setup(),voice=new VoiceCoach();voice.announceFinished('set-1');voice.announceTransition(45,undefined,'set-1');prepare(voice,'set-2');voice.announceStart('set-2');expect(spoken).toEqual(['Закончили.','Отдых 45 секунд.','Приготовьтесь. Пять.','Четыре','Три','Два','Один','Начали.'])})
  ...
  it.each([['timed',timed('Круговые движения тазом'), '30 секунд.'],['reps',reps('Приседания'), '15 раз.']])('announces next %s exercise and target at rest start',(_,next,target)=>{const{spoken}=setup(),voice=new VoiceCoach();voice.announceFinished('transition');voice.announceTransition(45,next as WorkoutExercise,'transition');expect(spoken).toEqual(['Закончили.',`Отдых 45 секунд. Следующее упражнение — ${(next as WorkoutExercise).name}. ${target}`])})
  it('handles reps to timed and reps to same reps set',()=>{const{spoken}=setup(),voice=new VoiceCoach();voice.announceTransition(60,timed(),'reps-timed');voice.announceTransition(60,undefined,'reps-same');expect(spoken).toEqual(['Отдых 60 секунд. Следующее упражнение — Планка. 30 секунд.','Отдых 60 секунд.'])})
