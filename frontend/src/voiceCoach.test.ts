...
describe("VoiceCoach",()=>{
  ...
  it("combines rest with the upcoming exercise",()=>{const {spoken}=setup();const voice=new VoiceCoach();voice.announceRest(60,"Подтягивания");expect(spoken).toEqual(["Отдых, следующее упражнение: Подтягивания."])});
  it("announces the first exercise before preparation countdown",()=>{const {spoken}=setup();const voice=new VoiceCoach();voice.preparationCountdown("first",5,"Круги плечами");[4,3,2,1].forEach(x=>voice.preparationCountdown("first",x));expect(spoken).toEqual(["Первое упражнение: Круги плечами. Приготовьтесь. Пять.","Четыре","Три","Два","Один"])});
  it("starts every later preparation countdown with a prompt",()=>{const {spoken}=setup();const voice=new VoiceCoach();[5,4,3,2,1].forEach(x=>voice.preparationCountdown("next",x));expect(spoken).toEqual(["Приготовьтесь. Пять.","Четыре","Три","Два","Один"])});
  ...
  it("cancels stale queued speech before every critical countdown value",()=>{const {speech}=setup();const voice=new VoiceCoach();voice.announceRest(30);voice.countdown("set",5);voice.countdown("set",4);expect(speech.cancel).toHaveBeenCalledTimes(2)});
  it("uses compact Russian exercise phrases",()=>{const {spoken}=setup();const voice=new VoiceCoach();voice.announceExercise("Отжимания",15);expect(spoken).toEqual(["Отжимания. 15 раз."])});
});
