...
import {actualTimedSetSeconds,addRestSeconds,elapsedSeconds,exerciseTarget,isPreparationTime,nextSet,progressPercent,skipRest,totalSets,workoutSummary} from './workoutFlow'
...
describe('workout flow',()=>{
  ...
  it('moves the main timer into preparation for its final five seconds',()=>{expect(isPreparationTime(6)).toBe(false);expect(isPreparationTime(5)).toBe(true);expect(isPreparationTime(4)).toBe(true)})
